...
export class NetworkConfig {
    ...
    static fromHttpResponse(payload: any): NetworkConfig {
        ...
        networkConfig.ChainID = String(payload["moa_chain_id"]);
        networkConfig.GasPerDataByte = Number(payload["moa_gas_per_data_byte"]);
        networkConfig.TopUpFactor = Number(payload["moa_top_up_factor"]);
        networkConfig.RoundDuration = Number(payload["moa_round_duration"]);
        networkConfig.RoundsPerEpoch = Number(payload["moa_rounds_per_epoch"]);
        networkConfig.TopUpRewardsGradientPoint = new BigNumber(payload["moa_rewards_top_up_gradient_point"]);
        networkConfig.MinGasLimit = Number(payload["moa_min_gas_limit"]);
        networkConfig.MinGasPrice = Number(payload["moa_min_gas_price"]);
        networkConfig.MinTransactionVersion = Number(payload["moa_min_transaction_version"]);
        networkConfig.GasPriceModifier = Number(payload["moa_gas_price_modifier"]);
